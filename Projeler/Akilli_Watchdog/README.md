...
Lead/içerik pipeline'larınızın 7/24 sağlıklı çalışıp çalışmadığını kontrol eder.
Form değişiklikleri, şema kaymaları ve veri kalitesi sorunlarını **proaktif olarak** tespit eder.
Sorun bulduğunda detaylı HTML raporu üretir, ama bu rapor doğrudan inbox'a düşmez —
`WatchdogInternal` Gmail label'ına auto-archive edilir. İstersen sabah bir
zamanlı routine bu label'ı tarayıp sorunları otomatik düzeltir; sadece
düzeltilemeyen sorunlar için kısa bir özet mail atar.

**Bu desen şuna yarar:** Birden fazla otomasyonu/cron'u olan herkes için.
Servisin çöktüğünü kullanıcıdan duymak yerine, sistem kendi kendini
denetler ve sadece gerçek bir sorun olunca sana haber verir.
...
Pakette iki örnek satır bırakıldı (biri Notion + Railway, biri sadece Railway).
Yeni bir proje eklemek için listeye bir `dict` entry eklemek yeterlidir.

| Pipeline tipi | Kontrol |
|-------|---------|
| `custom_notion` (Sheets/Notion → Railway) | DB erişim, Header/Property uyumu, Veri Kalitesi, Deploy durumu |
| `railway_only` (sadece Railway servisi) | Son deployment durumu + opsiyonel HTTP health-check |
...
Sorun tespit edildiğinde HTML rapor üretilir ama doğrudan kullanıcıya gitmez:
...
| ✅ **Sağlıklı** | Sorun yoksa `--force` ile tam rapor üretilebilir |

### Otonom tepki katmanı

Mail gönderildikten hemen sonra `alerter.py` Gmail API ile mesajdan
`INBOX` ve `UNREAD` label'larını kaldırır, `WatchdogInternal` label'ını
ekler (label yoksa otomatik oluşturur). Sonuç: rapor inbox'ta hiç görünmez.

İstersen bu label'ı tarayan bir zamanlı routine (Claude Code scheduled routine
veya cron) kurabilirsin. Routine son raporu okur, sorunları kategorize eder,
kod/git üzerinden düzeltebildiklerini commit + push eder. Sadece düzeltemediği
sorunlar varsa kısa bir `🚨 Watchdog: X sorun elle bakım istiyor` özet maili
gönderir. Hiç sorun yoksa veya hepsi düzeltildiyse hiçbir mail görmezsin.
...
| `ALERT_EMAIL` | Alarm maillerinin gideceği e-posta | ✅ |
| `NOTION_API_TOKEN` | Notion API token (Notion kontrolü için) | ⭐ |
| `NOTION_SOCIAL_TOKEN` | İkinci Notion workspace token'ı (opsiyonel) | ❌ |
| `NOTION_DATABASE_ID` | Varsayılan Notion DB ID | ❌ |
| `GOOGLE_SERVICE_ACCOUNT_JSON` | Service Account JSON (production auth) | 🔄 |
| `RAILWAY_TOKEN` | Railway deployment probe için API token | ⭐ |
| `NOTION_DB_ORNEK` | İzlenen örnek projenin Notion DB ID'si | ❌ |
| `RAILWAY_SERVICE_ID_ORNEK` | İzlenen örnek projenin Railway service ID'si | ❌ |
