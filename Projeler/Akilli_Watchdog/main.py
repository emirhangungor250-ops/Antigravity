...
from railway_log_checker import check_railway_logs
    ...
    for svc in services:
        ...
        try:
            ...
            if edges:
                ...
                # Railway deployment statuses: SUCCESS, FAILED, CRASHED, BUILDING, DEPLOYING, QUEUED, INITIALIZING, SKIPPED, etc.
                ...
                # QUEUED/INITIALIZING genelde saniyeler içinde SUCCESS'e döner; son 15 dk içindeyse "in progress" kabul et
                if not is_healthy and deploy_status in ("QUEUED", "INITIALIZING", "WAITING"):
                    try:
...
def check_http_health() -> list[dict]:
    """
    `http_health_url` tanımlı projeler için /healthz dış-pingi yapar.
    Railway deployment status SUCCESS olsa bile runtime crash sırasında 200 dönmeyebilir;
    bu kontrol o boşluğu kapatır.
    """
    results = []
    for project in Config.MONITORED_PROJECTS:
        url = project.get("http_health_url")
        if not url:
            continue
        try:
...
def run_health_check(force_email: bool = False) -> dict:
    ...
    # ── KATMAN 1c: Railway Log Tarama (son 24h ERROR'lar) ─────
    logger.info("── Katman 1c: Railway Log Tarama (son 24 saat) ──")
    railway_log_results = check_railway_logs(hours=24)
    for rl in railway_log_results:
        if rl.get("issue"):
            all_issues.append(rl["issue"])

    # ── KATMAN 1d: HTTP /healthz Probe ─────────────────────
    logger.info("── Katman 1d: HTTP /healthz Probe ──")
    http_results = check_http_health()
    for hr in http_results:
        if hr["healthy"]:
            logger.info(f"  ✅ {hr['name']} → HTTP {hr['status_code']}")
        else:
            detail = hr.get("error") or f"HTTP {hr['status_code']}"
            msg = f"🚨 [HTTP] {hr['name']} /healthz başarısız: {detail} ({hr['url']})"
            all_issues.append(msg)
            logger.critical(f"  {msg}")
    ...
    report = _build_result(all_issues, sheets_results, notion_results, llm_results,
                           token_results, railway_results, railway_log_results)
    ...
    send_alert_email(
        ...
        railway_log_results=railway_log_results,
    )
