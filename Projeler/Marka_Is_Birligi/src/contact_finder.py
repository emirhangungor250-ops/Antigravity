...
Pipeline (Waterfall):
0. Website Discovery → Heuristic + Apify Google Search ile resmi siteyi bul
...
try:
    from src.website_discovery import find_official_website
except ImportError:
    try:
        from website_discovery import find_official_website
    except ImportError:
        from .website_discovery import find_official_website
...
def find_contacts_for_brand(brand_info):
    ...
    # ─── KADEME 0: Website Discovery (heuristic + Apify Google Search) ───
    if not website:
        try:
            discovery = find_official_website(brand_name, handle)
            website = discovery.get("url", "")
            if website:
                src = discovery.get("source", "?")
                conf = discovery.get("confidence", "?")
                cost = discovery.get("apify_cost_usd", 0.0)
                print(f"  🌐 Layer 0: {website} (source={src}, confidence={conf}, apify=${cost:.4f})")
            else:
                print(f"  ⚠️ Layer 0: Web sitesi bulunamadı")
        except Exception as exc:
            logger.error(f"Layer 0 hatası, eski heuristic'e düşülüyor: {exc}", exc_info=True)
            website = guess_website_from_handle(handle)
            if website:
                print(f"  🌐 Web sitesi tahmin edildi (fallback): {website}")
