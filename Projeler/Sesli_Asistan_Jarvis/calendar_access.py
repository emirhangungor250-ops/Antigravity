...
def format_schedule_summary(events: list[dict]) -> str:
    """Format a brief voice-friendly summary of the schedule (Türkçe)."""
    if not events:
        return "Bugün takviminiz boş, efendim."

    count = len(events)
    if count == 1:
        evt = events[0]
        if evt.get("all_day"):
            return f"Bugün tüm gün süren bir etkinliğiniz var: {evt['title']}."
        return f"Bugün bir etkinliğiniz var: {evt['start']}'te {evt['title']}."

    summaries = []
    for evt in events[:5]:
        if evt.get("all_day"):
            summaries.append(f"tüm gün {evt['title']}")
        else:
            summaries.append(f"{evt['start']}'te {evt['title']}")

    result = f"Bugün {count} etkinliğiniz var, efendim. "
    result += ". ".join(summaries[:3])
    if count > 3:
        result += f". Ve {count - 3} tane daha."
    return result
