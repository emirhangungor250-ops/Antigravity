...
export function createOrb(canvas: HTMLCanvasElement): Orb {
  ...
  // ── State ──
  let state: OrbState = "asleep";
  // Wake/birth animasyonu: 0 = uykuda (görünmez/sönük), 1 = tam uyanık
  let birth = 0, targetBirth = 0;
  // Uyanma patlaması: wake() anında 1'e fırlar, ~1.5sn'de söner (parlama + genişleme burst'ü)
  let wakePulse = 0;
  ...
  function animate() {
    ...
    switch (state) {
      case "asleep":
        // Uykuda: küçük, sönük, çizgisiz
        targetRadius = 10; targetSpeed = 0.08; targetBright = 0.4; targetSize = 0.3;
        targetLineAmount = 0; targetElectronRate = 0; break;
      case "idle":
    ...
    // Wake/birth: uykudan doğuş — patlama anında çok hızlı dolar, sonra yumuşar
    const birthLerp = wakePulse > 0.15 ? 0.28 : 0.06;
    birth += (targetBirth - birth) * birthLerp;
    // Uyanma patlaması söner (~1sn) — daha çevik
    wakePulse *= 0.93;
    ...
    if (lineAmount > 0.01) {
      ...
      lp.needsUpdate = true;
      // birth çarpanı: uykuda/uyanırken çizgiler de fade-in olur
      lineMat.opacity = lineAmount * 0.12 * (birth * birth * (3 - 2 * birth));
    ...
    // birth ile fade-in: uykuda görünmez, uyanırken parlaklık + boyut artar
    const birthEase = birth * birth * (3 - 2 * birth); // smoothstep
    // Uyanma patlaması: ani parlama + büyük parçacıklar, sonra idle'a oturur
    const pulse = wakePulse * wakePulse; // keskin tepe
    mat.opacity = Math.min(1, (currentBright + bass * 0.08 + pulse * 1.1) * birthEase);
    mat.size = (currentSize + bass * 0.05 + pulse * 0.9) * (0.4 + birthEase * 0.6);
    // Tüm bulut patlamayla şişip geri çekilir (şok dalgası hissi)
    const wakeScale = 1 + pulse * 0.7;
    points.scale.setScalar(wakeScale);
    lines.scale.setScalar(wakeScale);
    electrons.scale.setScalar(wakeScale);
  ...
  return {
    setState(s: OrbState) {
      state = s;
      // Uykuda iken birth hedefi 0; uyandıktan sonra wake() 1'e çeker
      if (s === "asleep") { targetBirth = 0; birth = 0; }
    },
    ...
    wake() {
      targetBirth = 1;
      birth = 0.35;              // hemen belirgin başla, patlamayla hızla tam boya çık
      wakePulse = 1.0;           // parlama + genişleme burst'ünü tetikle
      transitionEnergy = 1.0;    // hızlı dönüş/tumble
      spinY += 0.25;             // uyanış anında keskin bir savrulma
    },
    destroy() {
      destroyed = true;
