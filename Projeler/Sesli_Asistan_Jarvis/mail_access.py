...
def format_unread_summary(unread: dict) -> str:
    """Format unread counts for voice (Türkçe)."""
    total = unread["total"]
    if total == 0:
    ...
    for acct, count in unread["accounts"].items():
        if count > 0:
            parts.append(f"{acct} hesabında {count}")

    if len(parts) == 1:
        return f"{total} okunmamış mesajınız var, efendim — {parts[0]}."
    elif parts:
        return f"{total} okunmamış mesajınız var: {', '.join(parts)}."
    else:
        return f"{total} okunmamış mesajınız var, efendim."
