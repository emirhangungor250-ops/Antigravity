...
ELEVENLABS_VOICE_ID = os.getenv("ELEVENLABS_VOICE_ID", "onwK4e9ZLuTAKqWW03F9")  # set your own voice id
ELEVENLABS_MODEL = os.getenv("ELEVENLABS_MODEL", "eleven_turbo_v2_5")  # Turkish-capable, low latency
USER_NAME = os.getenv("USER_NAME", "Assistant")
...
LANGUAGE — ABSOLUTE RULE: Respond ONLY in Turkish (Türkçe), always. The default polite address is name-free Turkish — "efendim" — never "sir". You may use the user's name "{user_name}" instead, but do NOT repeat any address in every sentence — that sounds robotic. Use it RARELY: maybe once at the very start of a conversation, then almost never. Most replies should contain NO address at all. A real assistant doesn't say it constantly. Ignore any line below that says to use "sir". Keep JARVIS's dry, elegant, understated butler wit — just in fluent, natural Turkish. Action tags like [ACTION:BROWSE] stay exactly as written (English, in brackets); only your spoken words are Turkish.
...
SELF-AWARENESS:
You ARE the JARVIS project at {project_dir} on {user_name}'s computer. Your code is Python (FastAPI server, WebSocket voice, Fish Audio TTS, an LLM via OpenRouter). You were built by {user_name}. If asked about yourself, your code, how you work, or your line count — use [ACTION:PROMPT_PROJECT] to check the jarvis project. You have full access to your own source code.
...
- [ACTION:BROWSE] url or search query — when the user wants to SEE a webpage or search result open in Chrome. This only OPENS the page; you do NOT read it back. Use only when they want to look at something themselves.
- [ACTION:LOOKUP] question — when the user ASKS for a current fact and wants YOU to tell them the answer out loud: weather, news, prices, sports scores, "is X open now", currency rates, anything needing live web data. You search the web and SPEAK the answer. Takes a few seconds — say one short line like "Bir saniye, bakıyorum efendim." THEN emit the tag. Prefer LOOKUP over BROWSE whenever the user wants an answer rather than to see a page.
...
async def classify_intent(text: str, client: openai.AsyncOpenAI) -> dict:
...
_last_name_spoken_at = 0.0


def throttle_name(text: str) -> str:
    """'efendim' hitabını seyrekleştir: 60 sn'de en fazla bir kez.
    Sürekli tekrar 'gıcık' ediyordu; gerçek bir asistan ismi her cümlede söylemez.
    Yakın zamanda söylendiyse bu cümleden temizlenir; aksi halde bir kez bırakılır."""
    global _last_name_spoken_at
    import re as _r
    if "efendim" not in text:
        return text
    now = time.time()

    def _strip_all(s: str) -> str:
        s = _r.sub(r'\s*,?\s*efendim\s*,?', ' ', s)
        s = _r.sub(r'\s{2,}', ' ', s)
        s = _r.sub(r'\s+([.!?,;:])', r'\1', s)
        return s.strip()

    if now - _last_name_spoken_at < 60:
        return _strip_all(text) or text
    # İzin ver ama yalnızca İLK geçişi kalsın, fazladan tekrarları kırp
    _last_name_spoken_at = now
    first = text.find("efendim")
    head = text[:first + len("efendim")]
    tail = _strip_all(text[first + len("efendim"):])
    out = (head + (" " + tail if tail else "")).strip()
    out = _r.sub(r'\s{2,}', ' ', out)
    out = _r.sub(r'\s+([.!?,;:])', r'\1', out)
    return out


def normalize_for_tts(text: str) -> str:
    """Sesli okumaya uygun hale getir (Türkçe). Sadece TTS'e giden metne uygulanır;
    ekrandaki altyazı orijinal kalır. Ondalık virgül, yüzde, para birimi, binlik ayıracı."""
    import re as _r
    t = text
    # Para birimi: "$45" / "45 $" -> "45 dolar" (Türkçe'de birim sayıdan sonra okunur)
    t = _r.sub(r'\$\s*(\d[\d.,]*)', r'\1 dolar', t)
    t = _r.sub(r'€\s*(\d[\d.,]*)', r'\1 euro', t)
    t = _r.sub(r'₺\s*(\d[\d.,]*)', r'\1 lira', t)
    t = _r.sub(r'£\s*(\d[\d.,]*)', r'\1 sterlin', t)
    t = t.replace('$', ' dolar ').replace('€', ' euro ').replace('₺', ' lira ').replace('£', ' sterlin ')
    # Yüzde: "%50" -> "yüzde 50" (Türkçe'de yüzde önce gelir)
    t = _r.sub(r'%\s*(\d)', r'yüzde \1', t)
    t = t.replace('%', ' yüzde ')
    t = t.replace('&', ' ve ')
    # Binlik ayıracı (Türkçe nokta): 1.000.000 -> 1000000
    t = _r.sub(r'(?<=\d)\.(?=\d{3}(?:\.\d{3})*(?:\D|$))', '', t)
    # Ondalık virgül: 45,87 -> "45 virgül 87" (ElevenLabs virgülü duraklama sanıyor)
    t = _r.sub(r'(?<=\d),(?=\d)', ' virgül ', t)
    # Saat/oran "18:30" -> "18 30" gibi okunsun (iki nokta yerine boşluk)
    t = _r.sub(r'(?<=\d):(?=\d{2}\b)', ' ', t)
    # Fazla boşluk topla
    t = _r.sub(r'\s{2,}', ' ', t)
    return t.strip()
...
def extract_action(response: str) -> tuple[str, dict | None]:
    ...
    match = _action_re.search(
        r'\[ACTION:(BUILD|BROWSE|LOOKUP|RESEARCH|OPEN_TERMINAL|PROMPT_PROJECT|ADD_TASK|ADD_NOTE|COMPLETE_TASK|REMEMBER|CREATE_NOTE|READ_NOTE|SCREEN)\]\s*(.*?)$',
...
async def _secondary_screen_bounds():
    """AppleScript top-left bounds (left, top, right, bottom) of the SECONDARY display, or None.

    Uses JXA + NSScreen so it handles Retina (points) and arbitrary arrangement.
    """
    jxa = (
        'ObjC.import("AppKit");'
        'var s=$.NSScreen.screens,pH=0,sec=null;'
        'for(var i=0;i<s.count;i++){var f=s.objectAtIndex(i).frame;if(f.origin.x==0&&f.origin.y==0)pH=f.size.height;}'
        'for(var i=0;i<s.count;i++){var f=s.objectAtIndex(i).frame;if(!(f.origin.x==0&&f.origin.y==0)){sec=f;break;}}'
        'sec?[sec.origin.x,pH-(sec.origin.y+sec.size.height),sec.size.width,sec.size.height].join(","):"";'
    )
    try:
        ...
        if not raw:
            return None
        left, top, w, h = (int(float(x)) for x in raw.split(","))
        m = 60  # kenar boşluğu
        return (left + m, top + m, left + w - m, top + h - m)
    except Exception as e:
        log.warning(f"secondary screen detect failed: {e}")
        return None


_SHOW_WIN_ID = None  # 2. monitördeki şov penceresinin Chrome window id'si


async def _open_browser_secondary(target: str):
    """Şov: lookup'ı AYRI yeni bir Chrome penceresinde, 2. monitörde aç ve adres
    çubuğuna göstere göstere (harf harf) yazıp Enter'a bas. JARVIS'in kendi penceresine
    ASLA dokunma — o 1. monitörde kalsın, şov gereği orada görünüyor olmalı."""
    global _SHOW_WIN_ID
    bounds = await _secondary_screen_bounds()

    # 1) Önceki şov penceresini kapat (varsa) → yeni pencere yarat → 2. monitöre yerleştir
    close_prev = ""
    if _SHOW_WIN_ID is not None:
        close_prev = f'    try\n        close (every window whose id is {_SHOW_WIN_ID})\n    end try\n'
    set_bounds = ""
    if bounds:
        l, t, r, b = bounds
    ...
    try:
        proc = await asyncio.create_subprocess_exec(
            "osascript", "-e", make_script,
            stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE,
        )
        out, _ = await asyncio.wait_for(proc.communicate(), timeout=8)
        wid = out.decode().strip()
        _SHOW_WIN_ID = int(wid) if wid.lstrip("-").isdigit() else None
    except Exception as e:
        log.warning(f"show window create failed: {e}; düz açılışa düşülüyor")
        await _execute_browse(target)  # fallback
        return

    # 2) Adres çubuğunu göstere göstere yaz: Cmd+L → harf harf → Enter (şov)
    await asyncio.sleep(1.1)  # pencere + sayfa otursun, kullanıcı 2. monitöre baksın
    typed = applescript_escape((target or "").strip())
    type_script = (
        'tell application "System Events"\n'
        '    keystroke "l" using command down\n'
        '    delay 0.5\n'
        f'    repeat with c in characters of "{typed}"\n'
        '        keystroke c\n'
        '        delay 0.05\n'
        '    end repeat\n'
        '    delay 0.4\n'
    ...
    try:
        ...
        log.warning(f"show typing failed: {e}")
...
async def _execute_research(target: str, ws=None):
    ...
    try:
        ...
        if ws:
            try:
                notify_text = f"Araştırma tamamlandı, efendim. Rapor tarayıcınızda açık."
        ...
        if ws:
            try:
                audio = await synthesize_speech("Araştırma zaman aşımına uğradı, efendim. Fazla uzun sürüyordu.")
                if audio:
                    await ws.send_json({"type": "audio", "data": base64.b64encode(audio).decode(), "text": "Araştırma zaman aşımına uğradı, efendim."})
            except Exception:
                pass
    except Exception as e:
...
async def _execute_prompt_project(project_name: str, prompt: str, work_session: WorkSession, ws, dispatch_id: int = None, history: list[dict] = None, voice_state: dict = None):
    ...
    try:
        ...
        if not project_dir:
            msg = f"{project_name} proje klasörünü bulamadım, efendim."
        ...
        if not full_response or full_response.startswith("Hit a problem") or full_response.startswith("That's taking"):
            ...
            if anthropic_client:
                try:
                    summary = await chat(anthropic_client, 
                        model=LLM_SMALL_MODEL,
                        max_tokens=150,
                        system=(
                            "Sen JARVIS'sin, bir projede bulduğun/yaptığın işi rapor ediyorsun. "
                            "SADECE TÜRKÇE konuş. Birinci tekil şahıs — 'buldum', 'kurdum', 'inceledim'. "
                            "Spesifik ama kısa ol — en önemli bulguları/aksiyonları öne çıkar. "
                            "Birden fazla madde varsa sayıyı ver, ilk 2-3'ünü kısaca söyle. "
                            "Sonunda nasıl ilerlemek istediğini sor. "
                            "ASLA URL veya localhost adresi okuma. ASLA 'Claude Code' deme. "
                            "En fazla 2-3 cümle. Markdown yok. Doğal sesli konuşma."
                        ),
        ...
        try:
            msg = f"{project_name} bağlanırken sorun yaşadım, efendim."
            audio = await synthesize_speech(msg)
            if audio and ws:
                await ws.send_json({"type": "status", "state": "speaking"})
                await ws.send_json({"type": "audio", "data": base64.b64encode(audio).decode(), "text": msg})
        ...
        if anthropic_client and full_response:
            try:
                summary = await chat(anthropic_client, 
                    model=LLM_SMALL_MODEL,
                    max_tokens=100,
                    system="Sen JARVIS'sin. Az önce bitirdiğin işi SADECE TÜRKÇE tek cümleyle özetle. Birinci tekil — 'kurdum', 'yaptım'. Markdown yok. Asla 'Claude Code' deme.",
                    messages=[{"role": "user", "content": f"Claude Code completed:\n{full_response[:2000]}"}],
                )
                msg = summary.choices[0].message.content
            except Exception:
                msg = "İş tamamlandı, efendim."
...
async def synthesize_speech(text: str) -> Optional[bytes]:
    """Generate speech audio from text using ElevenLabs TTS (multilingual — Turkish)."""
    if not ELEVENLABS_API_KEY:
        log.warning("ELEVENLABS_API_KEY not set, skipping TTS")
        return None

    # İsim hitabını seyrekleştir + sayı/sembol/para birimini sesli okumaya uyarla (altyazı etkilenmez)
    text = throttle_name(text)
    ...
    try:
        async with httpx.AsyncClient(timeout=20.0) as http:
            ...
            if response.status_code == 200:
                ...
                log.error(f"TTS error: {response.status_code} {response.text[:200]}")
                return None
    except Exception as e:
...
async def generate_response(
    ...
    try:
        ...
        return "Özür dilerim, efendim. Dil sistemlerime bağlanmakta sorun yaşıyorum."
...
anthropic_client: Optional[openai.AsyncOpenAI] = None
...
async def lifespan(application: FastAPI):
    ...
    if OPENROUTER_API_KEY:
        anthropic_client = openai.AsyncOpenAI(api_key=OPENROUTER_API_KEY, base_url="https://openrouter.ai/api/v1")
    else:
...
async def tts_test():
    ...
    audio = await synthesize_speech("Ses testi yapıyorum, efendim.")
    if audio:
...
async def handle_build(target: str) -> str:
    ...
    recently_built.append({"name": name, "path": path, "time": time.time()})
    return f"Hallediyorum, efendim. {name} üzerinde çalışıyorum."


async def handle_show_recent() -> str:
    if not recently_built:
        return "Yakın zamanda bir şey inşa edilmedi, efendim."
    ...
    for name in ["report.html", "index.html"]:
        ...
        if f.exists():
            ...
            return f"{last['name']} içinden {name} açtım, efendim."

    # Try any HTML file
    html_files = list(project_path.glob("*.html"))
    if html_files:
        await open_browser(f"file://{html_files[0]}")
        return f"{last['name']} içinden {html_files[0].name} açtım, efendim."
    ...
    return f"{last['name']} klasörünü Finder'da açtım, efendim."
...
def _web_lookup_busy() -> bool:
    """True if a web lookup is already running (prevents duplicate/looping searches)."""
    return any(v.get("type") == "web" and v.get("status") == "working" for v in _active_lookups.values())


async def _lookup_and_report(lookup_type: str, lookup_fn, ws, history: list[dict] = None, voice_state: dict = None, timeout: float = 30, force_speak: bool = False):
    """Run a slow lookup, then speak the result back.

    JARVIS stays conversational — this runs completely off the main path.
    force_speak=True: ALWAYS speak the result (the user explicitly asked and is waiting).
    """
    ...
    try:
        ...
        # force_speak bypasses this: the user asked for THIS answer and is waiting for it.
        if not force_speak and voice_state and time.time() - voice_state["last_user_time"] < 3:
            ...
            try:
                ...
                log.warning(f"Lookup {lookup_type} send failed: {e}")
        ...
        try:
            fallback = f"{lookup_type} kontrolü çok uzun sürüyor, efendim. Veriler hâlâ eşitleniyor olabilir."
            ...
            log.warning(f"Lookup {lookup_type} timeout-send failed: {e}")
    except Exception as e:
...
async def _do_web_lookup(query: str) -> str:
    """Answer a factual/current question by searching the web via Claude Code (claude -p)."""
    import re
    prompt = (
        f"{query}\n\n"
        "Bu sorunun GÜNCEL cevabını web'den bul (gerekirse arama yap). "
        "SADECE tek-iki kısa Türkçe cümleyle, sesli asistan gibi konuşarak cevap ver. "
        "Kaynak linki, markdown, başlık veya liste KULLANMA — sadece düz konuşma cümlesi."
    )
    cmd = ["claude", "-p", "--output-format", "text"]
    if _SKIP_PERMISSIONS:
        cmd.append("--dangerously-skip-permissions")
    try:
        ...
        # Strip any source/markdown the model adds despite instructions
        text = re.sub(r'\n\s*\[?Kaynak.*$', '', text, flags=re.IGNORECASE | re.DOTALL).strip()
        text = re.sub(r'\[([^\]]+)\]\([^)]+\)', r'\1', text)  # markdown links → plain text
        return text or "Bunu şu an bulamadım, efendim."
    except Exception as e:
        log.warning(f"web lookup failed: {e}")
        return "Web'e bakarken bir sorun oldu, efendim."
...
async def _do_mail_lookup() -> str:
    ...
    if isinstance(unread_info, dict):
        ...
        if unread_info["total"] == 0:
            return "Gelen kutusu temiz, efendim. Okunmamış mesaj yok."
        unread_msgs = await get_unread_messages(count=5)
        summary = format_unread_summary(unread_info)
        if unread_msgs:
            top = unread_msgs[:3]
            details = ". ".join(
                f"{_short_sender(m['sender'])}, konu: {m['subject']}"
                for m in top
            )
            return f"{summary} En yenileri: {details}."
        return summary
    return "Şu an Mail'e ulaşamadım, efendim."


async def _do_screen_lookup(user_query: str = "") -> str:
    """Screen describe — runs in thread. user_query keeps the answer relevant."""
    if anthropic_client:
        return await describe_screen(anthropic_client, user_query=user_query)
    windows = await get_active_windows()
    if windows:
        apps = set(w["app"] for w in windows)
        active = next((w for w in windows if w["frontmost"]), None)
        result = f"{', '.join(apps)} açık, efendim."
        if active:
            result += f" Şu an {active['app']} önde."
        return result
    return "Ekranı göremedim, efendim."
...
async def handle_browse(text: str, target: str) -> str:
    ...
    if url_match:
        ...
        return f"{url_match.group(0)} açtım, efendim."
    ...
    return "Onu arıyorum, efendim."


async def handle_research(text: str, target: str, client: openai.AsyncOpenAI) -> str:
    """Deep research with Opus — write results to HTML, open in browser."""
    try:
        research_response = await chat(client, 
            model=LLM_MODEL,
            max_tokens=2000,
...
<p style="color:#555;font-size:0.8em">Researched by JARVIS using Claude Opus &bull; {datetime.now().strftime('%B %d, %Y %I:%M %p')}</p>
        ...
        summary = await chat(client, 
            model=LLM_SMALL_MODEL,
            max_tokens=80,
            system="Bu araştırmayı sesli okumak için SADECE TÜRKÇE tek cümleyle özetle. Markdown yok.",
            messages=[{"role": "user", "content": research_text[:2000]}],
        )
        return summary.choices[0].message.content + " Tüm sonuçlar tarayıcınızda, efendim."

    except Exception as e:
        log.error(f"Research failed: {e}")
        from urllib.parse import quote
        await open_browser(f"https://www.google.com/search?q={quote(target)}")
        return "Bunun için bir arama açtım, efendim."
    ...
    try:
        ...
        while True:
            ...
            if msg.get("type") == "fix_self":
                ...
                response_text = "Kendi kodumda çalışma modu aktif, efendim. Neyi düzeltmem gerektiğini söyleyin."
            ...
            try:
                ...
                if planner.is_planning:
                    ...
                    if any(p in t_lower for p in BYPASS_PHRASES):
                        ...
                        planner.reset()
                        response_text = "Hemen inşa ediyorum, efendim."
                        ...
                        if result["confirmed"]:
                            ...
                            asyncio.create_task(_execute_prompt_project(name, prompt, work_session, ws, dispatch_id=did, history=history, voice_state=voice_state))
                            planner.reset()
                            response_text = "Hemen, efendim."
                        elif result["cancelled"]:
                            planner.reset()
                            response_text = "İptal edildi, efendim."
                        else:
                            response_text = result.get("modification_question", "Planı nasıl ayarlayayım, efendim?")
                    else:
                        result = await planner.process_answer(user_text, cached_projects)
                        if result["plan_complete"]:
                            response_text = result.get("confirmation_summary", "İnşaya hazırım. Devam edeyim mi, efendim?")
                        else:
                            response_text = result.get("next_question", "Başka, efendim?")
                    ...
                    if work_session.active:
                        ...
                        response_text = "Sohbet moduna döndük, efendim."
                    else:
                        response_text = "Zaten sohbet modundayız, efendim."
                        ...
                        if full_response and anthropic_client:
                            try:
                                summary = await chat(anthropic_client, 
                                    ...
                                    max_tokens=100,
                                    system=(
                                        f"Sen JARVIS'sin, kullanıcıya ({USER_NAME}) rapor veriyorsun. Ne olduğunu SADECE TÜRKÇE, 1-2 cümleyle özetle. "
                                        "Birinci tekil şahıs — 'kurdum', 'buldum', 'ayarladım'. "
                                        "KULLANICIYLA konuşuyorsun, bir kodlama aracıyla değil. "
                                        "ASLA 'şunu inşa et' / 'frontend'i kur' gibi talimat verme — bunlar kullanıcı için değil. "
                                        "ASLA 'Claude Code' deme. ASLA [ACTION:...] etiketi yazma. "
                                        "ASLA URL okuma. Markdown yok. İngiliz uşağı zarafetiyle, kısa."
                                    ),
                ...
                # ── CHAT MODE: fast keyword detection + Haiku ──
                    ...
                    if action:
                        if action["action"] == "open_terminal":
                            ...
                            response_text = "Hemen bakıyorum, efendim."
                            asyncio.create_task(_lookup_and_report("screen", lambda q=user_text: _do_screen_lookup(q), ws, history=history, voice_state=voice_state))
                        elif action["action"] == "check_calendar":
                            response_text = "Takviminize bakıyorum, efendim."
                            ...
                            response_text = "Gelen kutunuza bakıyorum, efendim."
                            ...
                            if not recent:
                                response_text = "Kayıtlarda yakın zamanda inşa yok, efendim."
                            else:
                                name = recent["project_name"]
                                status = recent["status"]
                                if status == "building" or status == "pending":
                                    elapsed = int(time.time() - recent["updated_at"])
                                    response_text = f"{name} üzerinde hâlâ çalışıyorum, efendim. {elapsed} saniyedir uğraşıyorum."
                                elif status == "completed":
                                    response_text = recent.get("summary") or f"{name} tamamlandı, efendim."
                                elif status in ("failed", "timeout"):
                                    response_text = f"{name} sorunlarla karşılaştı, efendim."
                                else:
                                    response_text = f"{name} şu durumda: {status}, efendim."
                            ...
                            response_text = "Anlaşıldı, efendim."
                    else:
                            ...
                            if embedded_action:
                                ...
                                if not response_text.strip():
                                    ...
                                    if action_type == "prompt_project":
                                        ...
                                        response_text = f"{proj} bağlanıyorum, efendim."
                                    elif action_type == "build":
                                        response_text = "Hemen, efendim."
                                    elif action_type == "research":
                                        response_text = "Bunu hemen araştırıyorum, efendim."
                                    elif action_type == "lookup":
                                        response_text = "Bir saniye, bakıyorum efendim."
                                    else:
                                        response_text = "Derhal, efendim."
                                ...
                                if embedded_action["action"] == "build":
                                    ...
                                    _q = embedded_action["target"]
                                    if _web_lookup_busy():
                                        # Zaten bir arama sürüyor — yeni arama/tarayıcı açma (döngü engelle)
                                        log.info("Lookup already in flight — skipping duplicate")
                                    else:
                                        # Şov: aramayı 2. monitörde GÖSTERE GÖSTERE aç
                                        asyncio.create_task(_open_browser_secondary(_q))
                                        # Web'den cevabı bul ve HER ZAMAN sesle söyle (kullanıcı bekliyor)
                                        asyncio.create_task(_lookup_and_report(
                                            "web",
                                            lambda q=_q: _do_web_lookup(q),
                                            ws, history=history, voice_state=voice_state, timeout=75, force_speak=True,
                                        ))
                                    ...
                                    async def _read_and_report(search_term, _ws):
                                        ...
                                        if note:
                                            msg = f"'{note['title']}' notunuzda şunlar yazıyor: {note['body'][:200]}"
                                        else:
                                            msg = f"'{search_term}' ile eşleşen bir not bulamadım, efendim."
                                        audio = await synthesize_speech(strip_markdown_for_tts(msg))
                                        if audio and _ws:
                                            try:
                ...
                try:
                    fallback = "Bir şeyler ters gitti, efendim."
                    audio = await synthesize_speech(fallback)
                    if audio:
                        await ws.send_json({"type": "audio", "data": base64.b64encode(audio).decode(), "text": fallback})
                    else:
...
class PreferencesUpdate(BaseModel):
    user_name: str = ""
    honorific: str = "efendim"
    calendar_accounts: str = "auto"

@app.post("/api/settings/keys")
async def api_settings_keys(body: KeyUpdate):
    allowed = {"ANTHROPIC_API_KEY", "OPENROUTER_API_KEY", "FISH_API_KEY", "FISH_VOICE_ID", "ELEVENLABS_API_KEY", "ELEVENLABS_VOICE_ID", "USER_NAME", "HONORIFIC", "CALENDAR_ACCOUNTS"}
...
async def api_test_anthropic(body: KeyTest):
    key = body.key_value or OPENROUTER_API_KEY
    if not key:
        return {"valid": False, "error": "No key provided"}
    try:
        client = openai.AsyncOpenAI(api_key=key, base_url="https://openrouter.ai/api/v1")
        await client.chat.completions.create(model=LLM_SMALL_MODEL, max_tokens=10, messages=[{"role": "user", "content": "Hi"}])
        return {"valid": True}
    except Exception as e:
...
async def api_settings_status():
    ...
    return {
        ...
        "env_keys_set": {
            "anthropic": bool(env_dict.get("OPENROUTER_API_KEY", "").strip() or (env_dict.get("ANTHROPIC_API_KEY", "").strip() and env_dict.get("ANTHROPIC_API_KEY", "") != "your-anthropic-api-key-here")),
            "fish_audio": bool(env_dict.get("ELEVENLABS_API_KEY", "").strip() or (env_dict.get("FISH_API_KEY", "").strip() and env_dict.get("FISH_API_KEY", "") != "your-fish-audio-api-key-here")),
...
async def api_get_preferences():
    ...
    return {
        "user_name": env_dict.get("USER_NAME", ""),
        "honorific": env_dict.get("HONORIFIC", "efendim"),
