...
├── gmail-outreach-token.json   ← emirhangungor220@gmail.com (birincil hesap)
└── gmail-ikincil-token.json    ← emirhangungor250@gmail.com (ikincil hesap)
```

### Kullanım
```python
import sys, os
# Paket kökü = bu dosyanın 3 üst klasörü (Projeler/<proje>/<dosya> → kök)
...
# Outreach / birincil hesap → "outreach"
gmail = get_gmail_service("outreach")
sheets = get_sheets_service("outreach")
drive = get_drive_service("outreach")

# İkincil hesap → "ikincil"
gmail = get_gmail_service("ikincil")
sheets = get_sheets_service("ikincil")
```
