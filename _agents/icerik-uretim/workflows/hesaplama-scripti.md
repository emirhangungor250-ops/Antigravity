...
# Hesaplama Scripti — Yatırım/Senaryo Hesabı
...
- **Skill:** `Projeler/<ICERIK_PROJESI>/skills/icerik-yazari/SKILL.md`
- **Hesap Makinesi:** `Projeler/<ICERIK_PROJESI>/tools/calculator.py`
- **Kur Aracı:** `Projeler/<ICERIK_PROJESI>/tools/currency.py`
- **Referans Scriptler:** `Projeler/<ICERIK_PROJESI>/reference-scripts/yatirim_hesaplama_scriptleri.md`

## Sabit Hesaplama Metrikleri

> 📌 Bu değerler `_agents/icerik-uretim/config/ornek-musteri.yaml` → `hesaplama_metrikleri` bölümünden alınır.
...
2. **Config'den metrikleri doğrula**
   - `_agents/icerik-uretim/config/ornek-musteri.yaml` → `hesaplama_metrikleri`
   - Güncel değerler burada merkezi olarak yönetilir

3. **Hesapla**
   ```bash
   python "Projeler/<ICERIK_PROJESI>/tools/calculator.py"
   ```
...
4. **Döviz çevir** (TL karşılığı için)
   ```bash
   python "Projeler/<ICERIK_PROJESI>/tools/currency.py"
   ```
