...
# Repo kokunu script konumundan turet (skill _skills/canli-yayina-al/scripts/ altinda).
...
case "$1" in
    ...
    create-service)
        if [ -z "$4" ]; then
            ...
            echo "   Örnek: ./railway.sh create-service abc-123 my-bot <KULLANICI>/my-bot main" >&2
            exit 1
        fi
        local_proj="$2"
        local_svc="$3"
    ...
    connect-repo)
        if [ -z "$3" ]; then
            ...
            echo "   Örnek: ./railway.sh connect-repo abc-123 <KULLANICI>/my-bot main" >&2
            exit 1
        fi
        local_svc_id="$2"
        local_repo="$3"
