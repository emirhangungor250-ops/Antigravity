...
| Gmail (Ikincil) | `GMAIL_SWC_CLIENT_ID`, `GMAIL_SWC_CLIENT_SECRET` | |
...
├── gmail-outreach-token.json   ← <EMAIL> (Gmail+Drive+Sheets)
├── gmail-swc-token.json        ← <EMAIL> (Gmail+Drive+Sheets)
...
```python
import sys, os
# Paket kök dizinini bul (projeden 3 üst klasör: Projeler/<proje>/<dosya> → kök)
antigravity_root = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
